...
# DQN
class DQN:
    def __init__(self, n_states, n_actions, batch_size=256, lr=1e-4, gamma=0.99, mem_size=int(1e5), learn_step=5, tau=1e-3):
...
def train(env, agent, n_episodes=2000, max_steps=1000, eps_start=1.0, eps_end=0.1, eps_decay=0.995, target=200,
    ...
    for idx_epi in pbar:
        ...
        score_avg = np.mean(score_hist[-length:])
        scoreavg.append(score_avg)
...
agent = DQN(n_states=num_states, n_actions=num_actions, batch_size=BATCH_SIZE, lr=LR, gamma=GAMMA, mem_size=MEMORY_SIZE, learn_step=LEARN_STEP, tau=TAU)
score_hist, scoreavg = train(env, agent, n_episodes=EPISODES, target=TARGET_SCORE, chkpt=SAVE_CHKPT)
